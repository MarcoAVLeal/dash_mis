...
path_dados = "C:\\Users\\marco\\OneDrive - Crefaz Financiamentos e Investimentos\\Projeto_Leads_Bitrix\\Dash_Leads_Britrix_V2\\dados\\"  
path_source = "C:\\Users\\marco\\OneDrive - Crefaz Financiamentos e Investimentos\\Projeto_Leads_Bitrix\\Dash_Leads_Britrix_V2\\"
  
}else{
  
  path_dados = "Z:\\Usuários\\Nathalia\\IndicadoresVendasBitrix\\"
  path_source = "D:\\OneDrive - Crefaz Financiamentos e Investimentos\\Projeto_Leads_Bitrix\\Dash_Leads_Britrix_V2\\"
}
source(file = paste0(path_source,"funcoes.R"),encoding = "UTF-8")
df                <- read.csv(file = paste0(path_dados,"report-bitrix.csv"),sep = ";")
names_df          <- c("ID", "Fase do negocio","Fechado","Data de inicio","Pessoa reponsavel ID","Criado em","Modificado em",
                       "Modificado por ID","Modificado por","Produto Crefaz","Valor do emprestimo","Desafio","Fase automacao",
                       "Origem do Cliente","Contato Fonte","Data prevista de fechamento","Data negociar","Data analisar",
                       "Data prospectar")
...
df                <- df %>% mutate("Origem do Cliente"  = ifelse(`Origem do Cliente` == "","Não identificada",`Origem do Cliente`),
                                   # "Fonte"              = ifelse(`Contato Fonte` =="",`Lead Fonte`,`Contato Fonte`),
                                    "Origem do Cliente1" = ifelse(`Contato Fonte` != "","Fonte", `Origem do Cliente`),
                                   "Data fechado"       = ifelse(Fechado == "Sim",`Data prevista de fechamento`, NA),
...
df$`Data de inicio`    <- as.Date(df$`Data de inicio`,format = "%d/%m/%Y")
...
if(TRUE){
   if(params$visao == "Lojas"){
    Total <- sum  
    #table_fases_group           <- data.frame(Var1 = NA,NOVO = 0, PROSPECTAR = 0, NEGOCIAR = 0, ANALISAR = 0, PAGO = 0, DESAFIO = 0,  Total = 0, `Taxa de Pagos` = 0)
...
# data_ref            <- format(x = max(dados$value),"%d/%m/%Y")
# calendario = pd.read_excel('C:\\Users\\CFZ\\Documents\\jupyter_projetos\\relatorio_esteira\\DCalendario.xlsx', sheet_name='Calendario')
# calendario = calendario[["Dia","Class Não Útil"]]
# calendario.reset_index(drop=True, inplace=True) 
# calendario.rename(columns = {"Dia":"DATA","Class Não Útil":"Dia_util"}, inplace=True)
# calendario["Class_Dia"] = calendario['Dia_util'].isnull().values
# calendario.drop("Dia_util",axis='columns', inplace=True)
# #calendario = calendario['Dia'].astype(str).str.replace('-','/')
# calendario["DATA"] = pd.to_datetime(calendario["DATA"])
# calendario["DATA"] = calendario["DATA"].astype(str)
# df_calendario = sc.createDataFrame(calendario)
# df_calendario = df_calendario.withColumn("DATA",to_date(col("DATA"), 'yyyy-MM-dd'))
# df_calendario.show()
    

#df1 <- df1 %>% filter(Lojas == "Loja CFZ São José")
  Total <- sum
    ...
    data_ref              <- format(x = max(dados2$value[dados2$Class_Dia]),"%d/%m/%Y")
    dados2               <- dados2 %>% filter(value == max(dados2$value[dados2$Class_Dia]))
    # dados2               <- dados2  %>% dplyr::mutate("Atributo" = ifelse(Atributo =="Criou Lead" & (`Origem do Cliente1` == "Fonte" | `Origem do Cliente1` == " "  | `Origem do Cliente1` == ""),NA,Atributo  ))
    # total_movimentacoes2 <- dados2 %>% dplyr::filter(!is.na(Atributo)) %>% nrow
    ...
    #dados$`Origem do Cliente` %>% unique
    # dados   <- dados %>% dplyr::mutate("Atributo" = ifelse(Atributo == "Criou Lead" & (`Origem do Cliente1` == "Fonte" | `Origem do Cliente1` == " "  | `Origem do Cliente1` == ""),NA,Atributo  ))
    
    dados               <- dados %>% filter(value == max(dados2$value[dados2$Class_Dia]))
    total_movimentacoes <- dados %>%  nrow
#table(dados$Atributo,dados$`Origem do Cliente`)
    
    # table(dados$Atributo,dados$`Origem do Cliente1`)
    # 
    # which(dados$`Origem do Cliente` == " " )
```
...
```{r,echo=FALSE}
  if(params$visao == "Lojas"){
    #  Total <- sum
    #   dados <- df1 %>% dplyr::select(`Data criado`,`Data fechado`, `Data analisar`, `Data negociar`,ID_LEADS) %>%
    #   reshape2::melt(id.vars = "ID_LEADS",measure.vars = c("Data criado","Data fechado", "Data analisar", "Data negociar")) %>%
    #   mutate("Atributo"      = ifelse(variable == "Data criado","Criou Lead",
    #                                   ifelse(variable == "Data negociar","Moveu para negociar",
    #                                          ifelse(variable == "Data analisar","Moveu para analisar",
    #                                                ifelse(variable == "Data fechado","Fechou lead(Desafio ou pago)",variable)))))
    ...
    # 
    # dados <- left_join(completedados,df1,by = c("ID_LEADS"))
    # dados <- dados %>% filter(value == max(value))
    #   
